...
import { ConsultarUbicacionesActivas } from "../connections/comun/UbicacionConnection";
...
import { Ubicacion } from "../interfaces/comun/UbicacionInterface";
...
export const Settings = ( { open, setOpen, setTurno }: Props ) => {
    ...
    const [ubicacionArray, setUbicacionArray] = useState<Ubicacion[]>([]);
    ...
    const handleGuardarConfiguracion = async () => {
        ...
        await ActualizarUsuario({ ubicacion_id: ubicacion, turnos_tipos_ids: tiposTurno }).then( resp => {
            ...
            if( success ){
                ...
                setTimeout(() => {
                    ...
                    const data: stateProps = {
                        ...
                        rol: rol,
                        ubicacion: `Ventanilla ${ ubicacion.numero }`,
                        unidad: unidad
                    };
    ...
    useEffect(() => {
        ...
        async function obtener(){
            ...
            await ConsultarUbicacionesActivas().then( resp => {
                setUbicacionArray( resp.data );
            });
        }
    ...
    useEffect(() => {
        ...
        async function obtener(){
            ...
            await ConsultarConfiguracionUsuario().then( resp => {
                ...
                if( resp.data ){    
                    ...
                    const { ubicacion, turnos_tipos } = resp.data;        

                    if( ubicacion.id !== 0 ){
                        setUbicacion( ubicacion.id );
                    }
        ...
        if( open && ubicacionArray.length !== 0 && tiposTurnoArrayTemp.length !== 0 ){
            obtener();
        }       

    }, [ open, ubicacionArray, tiposTurnoArrayTemp ])    
    ...
    return (        
        ...
        >
                    ...
                    <Grid size={{ xs: 12, md: 12 }}>
                        ...
                        <FormControl fullWidth>
                            ...
                            <TextField 
                                ...
                                value={ ubicacion }
                                onChange={ (e) => setUbicacion( parseInt( e.target.value ?? '0' ) ) }
                            >   
                                <MenuItem key={0} value={0}>Seleccione una opción</MenuItem>      
                                {
                                    ubicacionArray?.map( ( { id, nombre, numero } ) => (
            ...
            <DialogActions>
                ...
                <Button disabled={ ubicacion === 0 || tiposTurno.length === 0 } variant="contained" onClick={ handleGuardarConfiguracion } loading={ loading }>
                    Guardar
                </Button>
